        ...
        <el-row class="diagnosis-header" type="flex">
          <el-col :span="12" style="display: flex; align-items: center">
            <div
              ...
              "
            ></div>
            <span class="header-title">{{ itemData.name }}</span>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    saveForm() {
      ...
      if (this.itemData.listid) {
        ...
        this.itemData.child.map((item) => {
          idArr.push(item.id),
            valueArr.push(
              `${item.F_Observation}|${item.F_FollowGuide}|${item.F_IndependentCompletion}|${item.F_Remark}`
            );
        });
        let data = {
    ...
    remarkSave() {
      this.itemData.child = this.itemData.child.map((item) => ({
        ...item,
        F_Remark: item.id == this.remarkid ? this.textarea : item.F_Remark,
      }));
